/**
 * @file detail.cc Implementation of Detail jrxml node
 *
 ...
 */
casper::jrxml::Detail::Detail (Node* a_parent) : Node(a_parent)
{
    node_type_ = EDetailBand;
}

/**
 * @brief Destructor
 */
casper::jrxml::Detail::~Detail ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::Detail::ParseXml (pugi::xml_node a_xml_node)
{
    /*
    ...
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {
        ...
        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("Detail element children must be a Band, %s is not allowed", child.name());
        }
        node = ElementFactory(child.name(), child, this);
        node->node_type_ = EDetailBand;
    }
    return true;
}

/**
 * @brief Factory method to create a Detail from it's XML representation
 *
 * @param a_xml_node DOM node the with XML spec
 * @param a_parent parent node of the jrxml
 * @return NULL if the element is not present in the DOM node
 */
casper::jrxml::Detail* casper::jrxml::Detail::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( nullptr != a_xml_node ) {
        casper::jrxml::Detail* node = new casper::jrxml::Detail(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::Detail::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] Detail\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
