/**
 * @file last_page_footer.cc JRXML that contains the last page footer band sub-bands.
 *
 ...
 */
casper::jrxml::LastPageFooter::LastPageFooter (Node* a_parent) : Node(a_parent)
{
    node_type_ = ELastPageFooter;
 ...
 * @brief Destructor
 */
casper::jrxml::LastPageFooter::~LastPageFooter ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::LastPageFooter::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("LastPageFooter element children must be a Band, %s is not allowed", child.name());
        }
        LastPageFooterBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::LastPageFooter* casper::jrxml::LastPageFooter::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::LastPageFooter* node = new casper::jrxml::LastPageFooter(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::LastPageFooter::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] LastPageFooter\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
