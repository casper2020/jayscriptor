/**
 * @file summary.cc JRXML that contains the summary band sub-bands.
 *
 ...
 */
casper::jrxml::Summary::Summary (Node* a_parent) : Node(a_parent)
{
    node_type_ = ESummary;
 ...
 */
bool casper::jrxml::Summary::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("Summary element children must be a Band, %s is not allowed", child.name());
        }
        SummaryBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::Summary* casper::jrxml::Summary::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::Summary* node = new casper::jrxml::Summary(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::Summary::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] Summary\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
