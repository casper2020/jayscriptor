/**
 * @file column_footer.cc JRXML Container for ColumnFooter sub-bands.
 *
 ...
 */
casper::jrxml::ColumnFooter::ColumnFooter (Node* a_parent) : Node(a_parent)
{
    node_type_ = EColumnFooter;
 ...
 * @brief Destructor
 */
casper::jrxml::ColumnFooter::~ColumnFooter ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::ColumnFooter::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("ColumnFooter element children must be a Band, %s is not allowed", child.name());
        }
        ColumnFooterBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::ColumnFooter* casper::jrxml::ColumnFooter::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::ColumnFooter* node = new casper::jrxml::ColumnFooter(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::ColumnFooter::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] ColumnFooter\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
