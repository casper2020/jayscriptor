/**
 * @file page_footer.cc JRXML Container for PageFooter sub-bands.
 *
 ...
 */
casper::jrxml::PageFooter::PageFooter (Node* a_parent) : Node(a_parent)
{
    node_type_ = EPageFooter;
 ...
 * @brief Destructor
 */
casper::jrxml::PageFooter::~PageFooter ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::PageFooter::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("PageFooter element children must be a Band, %s is not allowed", child.name());
        }
        PageFooterBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::PageFooter* casper::jrxml::PageFooter::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::PageFooter* node = new casper::jrxml::PageFooter(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::PageFooter::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] PageFooter\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
