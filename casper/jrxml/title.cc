/**
 * @file title.cc Contains the definition of the report title can contain one or more bands.
 *
 ...
 */
casper::jrxml::Title::Title (Node* a_parent) : Node(a_parent)
{
    node_type_ = ETitle;
 ...
 */
bool casper::jrxml::Title::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("Title element children must be a Band, %s is not allowed", child.name());
        }
        TitleBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::Title* casper::jrxml::Title::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::Title* node = new casper::jrxml::Title(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::Title::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] Title\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
