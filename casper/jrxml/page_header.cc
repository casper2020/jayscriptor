/**
 * @file page_header.cc JRXML Container for PageHeader sub-bands.
 *
 ...
 */
casper::jrxml::PageHeader::PageHeader (Node* a_parent) : Node(a_parent)
{
    node_type_ = EPageHeader;
 ...
 * @brief Destructor
 */
casper::jrxml::PageHeader::~PageHeader ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::PageHeader::ParseXml (pugi::xml_node a_xml_node)
{
    for (pugi::xml_node child = a_xml_node.first_child(); child; child = child.next_sibling()) {

        if ( strcmp(child.name(), "band") ) {
            throw OSAL_EXCEPTION("PageHeader element children must be a Band, %s is not allowed", child.name());
        }
        PageHeaderBand::CreateFromXml(child, this);
    }
    return true;
}
 ...
 */
casper::jrxml::PageHeader* casper::jrxml::PageHeader::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::PageHeader* node = new casper::jrxml::PageHeader(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::PageHeader::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] PageHeader\n", a_depth * 2, a_depth * 2, " ", child_index_);
}
