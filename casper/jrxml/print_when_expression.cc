/**
 * @file print_when_expression.cc Implementation of PrintWhenExpression model node
 *
 ...
 */
casper::jrxml::PrintWhenExpression::PrintWhenExpression (Node* a_parent) : Node(a_parent)
{
    node_type_ = EPrintWhenExpression;
 ...
 * @brief Destructor
 */
casper::jrxml::PrintWhenExpression::~PrintWhenExpression ()
{
    /* empty */
}
 ...
 */
bool casper::jrxml::PrintWhenExpression::ParseXml (pugi::xml_node a_xml_node)
{
 ...
 */
casper::jrxml::PrintWhenExpression* casper::jrxml::PrintWhenExpression::CreateFromXml (pugi::xml_node a_xml_node, Node* a_parent)
{
    if ( a_xml_node != NULL ) {
        casper::jrxml::PrintWhenExpression* node = new casper::jrxml::PrintWhenExpression(a_parent);
        if ( NULL != node ) {
 ...
 */
void casper::jrxml::PrintWhenExpression::PrintNodeInfo (FILE* a_stream, int a_depth) const
{
    fprintf(a_stream, "%*.*s[%d] PrintWhenExpression '%s'\n", a_depth * 2, a_depth * 2, " ", child_index_, (const char*) expression_);
}
